 ...
 *
 * This function is called to persist all AI-generated content 
 * to Convex in a single atomic operation.
 *
 * Workflow Context:
 * - Called after parallel AI generation jobs complete
...
export async function saveResultsToConvex(
  ...
  await convex.mutation(api.projects.updateProjectContent, {
    projectId,
    keyMoments: results.keyMoments ? JSON.stringify(results.keyMoments) : undefined,
    summary: results.summary ? JSON.stringify(results.summary) : undefined,
    socialPosts: results.socialPosts ? JSON.stringify(results.socialPosts) : undefined,
    titles: results.titles ? JSON.stringify(results.titles) : undefined,
    hashtags: results.hashtags ? JSON.stringify(results.hashtags) : undefined,
    youtubeTimestamps: results.youtubeTimestamps ? JSON.stringify(results.youtubeTimestamps) : undefined,
  });
